...
func TestRegisterNextIntervalCronExpression(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			minExpectedDur: 1 * time.Second,  // At least 1 second to allow for execution
...
func TestCronTimingConsistency(t *testing.T) {
	...
	baseTime := time.Date(2025, 11, 8, 14, 1, 0, 0, time.UTC) // 14:01:00
	testTimes := []time.Time{
	...
	expectedNextExecution := time.Date(2025, 11, 8, 15, 1, 0, 0, time.UTC) // 15:01:00
...
func TestMinimumDurationSafetyCheck(t *testing.T) {
	...
	now := time.Date(2025, 11, 8, 14, 5, 59, 900000000, time.UTC) // 14:05:59.9 (100ms before next minute)

	nextTime := schedule.Next(now)
	duration := nextTime.Sub(now) // Use Sub instead of time.Until to use mock time

	// If duration is less than 1 second, we should get the next occurrence
	if duration < time.Second {
		nextNextTime := schedule.Next(nextTime.Add(time.Second))
		nextDuration := nextNextTime.Sub(now) // Use Sub instead of time.Until
